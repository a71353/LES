...
from questionarios.filters import EstadosFilter
from .forms import *
from .models import *
from utilizadores.models import *
from inscricoes.models import Inscricao, Inscricaosessao, Inscricaotransporte
from atividades.models import Tema, TemaQ
...
from configuracao.tables import CursoTable, DepartamentoTable, DiaAbertoTable, EdificioTable, MenuTable, TemaTable, TransporteTable, UOTable, TemaQTable
from django_tables2 import SingleTableMixin, SingleTableView
from django_filters.views import FilterView
from configuracao.filters import CursoFilter, DepartamentoFilter, DiaAbertoFilter, EdificioFilter, MenuFilter, TemaFilter, TransporteFilter, UOFilter, TemaQFilter
from django.db.models.query import QuerySet
from questionarios.forms import EstadoQuestionarioForm  # Você precisará criar este formulário
from questionarios.models import EstadoQuestionario
...
class verTemasQ(SingleTableMixin, FilterView):

	table_class = TemaQTable
	template_name = 'configuracao/listaTemasQ.html'
	filterset_class = TemaQFilter
	table_pagination = {
		'per_page': 10
	}
	@handle_db_errors
...
def eliminarTemaQ(request, id):
    ...
    if tema.pergunta_set.exists():
        return redirect('utilizadores:mensagem3', 21)
...
def adicionarEstado(request):
    ...
    if request.method == 'POST':
        form = EstadoQuestionarioForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('configuracao:verEstados')
        else:
            # Redirect if the form is not valid
            return redirect('utilizadores:mensagem4', 22)
    else:
        form = EstadoQuestionarioForm()
...
def configurarEstado(request, id):
    ...
    estado = get_object_or_404(EstadoQuestionario, id=id)

    if request.method == 'POST':
        form = EstadoQuestionarioForm(request.POST, instance=estado)
        if form.is_valid():
            form.save()
            return redirect('configuracao:verEstados')
        else:
        ...
        form = EstadoQuestionarioForm(instance=estado)
