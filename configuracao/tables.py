...
from questionarios.models import Pergunta
...
class TemaQTable(tables.Table):
    ...
    def render_acoes(self, record):
        # Verificar se o tema está sendo usado em alguma pergunta
        tema_em_uso = Pergunta.objects.filter(tema=record).exists()
        ...
        if tema_em_uso:
            ...
            return format_html(f"""
                <div>
                    <a id='edit' href="{reverse('configuracao:editarTemaQ', kwargs={'id': record.pk})}">
                        <span class="icon is-small">
                            <i class="mdi mdi-circle-edit-outline mdi-24px"></i>
                        </span>
                    </a>
                    &nbsp;
                    <a onclick="alert.render('Tem a certeza que pretende eliminar este tema?','{reverse('configuracao:eliminarTemaQ', kwargs={'id': record.pk})}')">
                        <span class="icon is-small">
                            <i class="mdi mdi-trash-can-outline mdi-24px" style="color: #ff0000"></i>
                        </span>
                    </a>
...
class DiaAbertoTable(tables.Table):
    ...
    def before_render(self, request):
        ...
        self.columns.hide('questionario')
